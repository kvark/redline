impl Game {
    ...
    fn recover(&mut self) {
        let pose = self.vehicle.pose(&self.engine);
        let up = pose.position.normalize_or_zero();
        let fwd = crate::helpers::project_tangent(pose.orientation * glam::Vec3::Z, up);
        let next = Isometry {
    ...
    fn populate_hud(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(format!("Time  {}", crate::helpers::format_time(self.race.time)));
        if let Some(best) = self.race.best_lap {
            ui.label(format!("Best  {}", crate::helpers::format_time(best)));
        }
    ...
    fn follow_camera(&mut self) -> blade_engine::FrameCamera {
        ...
        // Enable with RUST_LOG=debug (or RUST_LOG=redline=debug).
